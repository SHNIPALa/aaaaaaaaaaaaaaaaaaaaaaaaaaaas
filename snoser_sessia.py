...
RECEIVERS = ['abuse@telegram.org', 'security@telegram.org', 'support@telegram.org']

DEVICES = [
    {"model": "iPhone 15 Pro", "system": "iOS 17.3"},
    {"model": "iPhone 14 Pro", "system": "iOS 17.2"},
    {"model": "Samsung S24 Ultra", "system": "Android 14"},
    {"model": "Samsung S23 Ultra", "system": "Android 13"},
]

OAUTH_SITES = [
    {"url": "https://my.telegram.org/auth/send_password", "phone_field": "phone"},
    {"url": "https://web.telegram.org/k/api/auth/sendCode", "phone_field": "phone"},
]
...
COMPLAINT_TEXTS_CHANNEL = {
    "1": {"subject": "Report: Violation", "body": "Channel {channel_link} violates rules.\nEvidence: {violation_link}"},
}
...
async def create_single_session(session_file: str, idx: int) -> dict:
    try:
        ...
        return {"client": client, "in_use": False, "flood_until": 0, "last_used": 0}
    except:
        return None
...
async def get_user_sessions_batch(user_id: int, count: int) -> list:
    ...
    available = [s for s in user_sessions[user_id]["sessions"] 
                 if not s["in_use"] and s["flood_until"] < now and now - s["last_used"] >= 2]
...
async def snos_attack_username(user_id: int, username: str, rounds: int, stop_event: asyncio.Event, progress_callback=None) -> int:
    ...
    for rnd in range(1, rounds + 1):
        ...
        sessions = await get_user_sessions_batch(user_id, 20)
        if not sessions:
            break
    ...
    if os.path.exists(BANNER_FILE):
        try:
            if hasattr(event, 'answer_photo'):
                return await event.answer_photo(FSInputFile(BANNER_FILE), caption=full_text, reply_markup=markup)
            else:
                return await bot.send_photo(user_id, FSInputFile(BANNER_FILE), caption=full_text, reply_markup=markup)
        except:
            pass
...
async def bomber_count(msg: types.Message, state: FSMContext):
    ...
    stop_event = asyncio.Event()
    active_bombers[user_id] = stop_event
...
async def mail_acc_type(cb: types.CallbackQuery, state: FSMContext):
    ...
    await cb.message.delete()
    await cb.message.answer("<b>VICTIM SNOS</b>\n\nВведите username:", 
                          reply_markup=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="Отмена", callback_data="mail_menu")]]))
...
async def mail_acc_id(msg: types.Message, state: FSMContext):
    ...
    sent = email_sender.send_mass(RECEIVERS, subject, body)
    await st.delete()
...
async def report_menu(cb: types.CallbackQuery, state: FSMContext):
    await state.set_state(ReportMessageState.waiting_link)
    await cb.message.delete()
    await cb.message.answer("<b>VICTIM SNOS</b>\n\nВведите ссылку на сообщение:", 
                          reply_markup=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="Отмена", callback_data="snos_type")]]))
